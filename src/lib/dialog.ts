  ...
  return new Promise((resolve) => {
    ...
    overlay.innerHTML = `
      <div style="background:var(--color-surface);border-radius:0.75rem;padding:1.5rem;max-width:420px;width:100%;box-shadow:0 25px 50px rgba(0,0,0,0.5)">
        ${title ? `<h2 style="margin:0 0 0.5rem;font-size:1.125rem;font-weight:600">${title}</h2>` : ''}
        <p style="margin:0 0 1.5rem;color:var(--color-text-muted);font-size:0.925rem;line-height:1.5">${message}</p>
        <div style="display:flex;gap:0.75rem;justify-content:flex-end">
          <button id="dlg-cancel" style="padding:0.5rem 1rem;border-radius:0.5rem;border:none;cursor:pointer;background:rgba(255,255,255,0.1);color:inherit;font-size:0.875rem">${cancelLabel}</button>
          <button id="dlg-confirm" style="padding:0.5rem 1rem;border-radius:0.5rem;border:none;cursor:pointer;background:${danger ? 'var(--color-primary)' : '#3b82f6'};color:#fff;font-size:0.875rem;font-weight:500">${confirmLabel}</button>
        </div>
      </div>
    `;
    ...
    overlay.innerHTML = `
      <div style="background:var(--color-surface);border-radius:0.75rem;padding:1.5rem;max-width:420px;width:100%;box-shadow:0 25px 50px rgba(0,0,0,0.5)">
        ...
        <div style="display:flex;justify-content:flex-end">
          <button id="dlg-ok" style="padding:0.5rem 1rem;border-radius:0.5rem;border:none;cursor:pointer;background:#3b82f6;color:#fff;font-size:0.875rem;font-weight:500">OK</button>
        </div>
      </div>
    `;
